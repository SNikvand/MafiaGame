// latest version: 03/30/14 - user list refreshes only at certain times such as connection,
//                            room creation, room joining and disconnection
...
function joinRoom(usersock, hostid) {
    ...
    usersock.emit('updateUserList');
    usersock.broadcast.to(hostid).emit('updateUserList');
...
function handleNameChangeAttempts(socket, nickNames, namesUsed) {
    socket.on('nameAttempt', function(name, avatar) {
        ...
        socket.broadcast.to(-1).emit('updateUserList');
    });
}
...
function handleRoomJoining(usersock) {
    usersock.on('joinGame', function(hostid) {
        ...
        if (currentRoomPlayer[hostid].length < currentRoomLimit[hostid]) { // if number of players is less than max
            ...
            usersock.broadcast.to(currentRoom[usersock.id]).emit('updateUserList');
...
function handleCreateRoom(socket) {
    socket.on('createGame', function(room) {
        ...
        socket.broadcast.to(currentRoom[socket.id]).emit('updateUserList');
