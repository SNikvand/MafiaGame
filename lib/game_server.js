...
exports.listen = function(server) {
    ...
    io.sockets.on('connection', function(socket) {
        ...
        // sends the list of rooms (host socket ids, along with their nicknames) and their names to populate the room list
...
/*
Chat room bug fix. Originally it was trying to use message.room for chat but changed to something more secure to
currentRoom[socket.id]. This needs to be reimplemented using redis.
 */
function handleMessageBroadcasting(socket) {
    socket.on('message', function(message) {
        ...
        console.log(currentRoom[socket.id] + ' --- ' + message.text);
    });
}
...
function handleRoomJoining(usersock) {
    usersock.on('joinGame', function(hostid) {
        /*if (dbaccess.gameHasStarted(hostid) == '1') {
...
function validateName(item) {
    ...
    var isValid = validName.test(item);
 ...
 /*--------------------------------------------------------------------------------
 Purpose: Sends message globally.
 ...
 -------------------------------------------------------------------------------*/
function unicastMessage(socket, message) {
